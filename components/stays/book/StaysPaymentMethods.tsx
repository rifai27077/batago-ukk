...
import { CreditCard, Wallet, Building2, CheckCircle2, Hotel } from "lucide-react";
import { useState } from "react";

type PaymentMethod = "credit_card" | "bank_transfer" | "pay_at_hotel";

export default function StaysPaymentMethods() {
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
        ...
        {selectedMethod === "pay_at_hotel" && (
            <div className="bg-green-50 border border-green-100 rounded-xl p-4 flex gap-3">
             ...
             <div>
                <p className="font-bold text-green-900 text-sm">Pay at Hotel</p>
                <p className="text-xs text-green-700 mt-1">No upfront payment required. You will pay the full amount when you arrive at the property.</p>
                <p className="text-xs font-bold text-green-800 mt-2">Credit card required for guarantee only.</p>
             </div>
          </div>
        )}
      </div>
